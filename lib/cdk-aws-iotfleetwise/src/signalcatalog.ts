...
export interface SignalCatalogSensorProps {
  ...
  readonly description?: string;
  readonly fullyQualifiedName: string;
  readonly structFullyQualifiedName?: string;
  readonly max?: number;
  readonly min?: number;
...
export interface SignalCatalogCustomStructProps {
  readonly description?: string;
  readonly fullyQualifiedName: string;
}

export interface SignalCatalogCustomPropertyProps {
  readonly dataType: string;
...
export class SignalCatalogCustomStruct extends SignalCatalogNode {
  constructor(props: SignalCatalogCustomStructProps) {
    super();
...
export class SignalCatalogCustomProperty extends SignalCatalogNode {
  constructor(props: SignalCatalogCustomPropertyProps) {
    super();
...
export class SignalCatalog extends Construct {
  ...
  constructor(scope: Construct, id: string, props: SignalCatalogProps) {
    ...
    this.arn = `arn:aws:iotfleetwise:${REGION}:${cdk.Aws.ACCOUNT_ID}:signal-catalog/${this.name}`;
    ...
    function parseVss(
      ...
      for (const node in vssNodes.nodes) {
        ...
        switch (vssNodes.nodes[node].type) {
          ...
          case 'property':
            signals.push(
              new SignalCatalogCustomProperty({
                ...vssSignal,
              }),
            );
            break;
    ...
    function vssDatatypeToFleetwise(
      ...
      | SignalCatalogBranch
      | SignalCatalogSensor
      | SignalCatalogCustomStruct
      | SignalCatalogCustomProperty {
